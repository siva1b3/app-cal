...
const RABBITMQ_HOST = "rabbitmq_service";
...
app.post("/calculate", async (req, res) => {
  const { num1, num2, operation, clientId } = req.body;
  ...
  let operation_id = undefined;
  try {
    const res = await pool.query(
      "INSERT INTO operations (operation_type, number_1, number_2) VALUES ($1, $2, $3) RETURNING operation_id",
      [operation, num1, num2]
    );
    operation_id = res.rows[0].operation_id;
    console.log("operation_id is:-", operation_id);
  } catch (error) {
  ...
  if (!operation_id) {
    return res
      .status(500)
      .json({ error: "something is wrong with the request" });
  }

  const request_status = await sendMessage(
    { num1, num2, operation, clientId, operation_id },
...
async function storeResult(result) {
  const { result: operation_result, operation_id } = result;
  try {
    await pool.query(
      "UPDATE operations SET operation_result = $1 WHERE operation_id = $2",
